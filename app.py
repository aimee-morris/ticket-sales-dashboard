...
def extract_metrics_from_pdf(pdf, filename):
    ...
    try:
        total_tickets = re.search(r"Cumulative\s+Ticket.*?(\d{1,3}(?:,\d{3})*)", text)
        gross = re.search(r"£\s*([\d,]+\.\d{2})", text)
        cap_match = re.search(r"(\d{1,3}(?:,\d{3})*)\s+\d{1,3}(?:,\d{3})*\.?\d*£.*?\%", text)
        ...
        data["Venue Capacity"] = int(cap_match.group(1).replace(",", "")) if cap_match else None
...
if uploaded_files:
    ...
    df["Weeks Remaining"] = ((df["Target Report Date"] - today).dt.days / 7).round(1)
